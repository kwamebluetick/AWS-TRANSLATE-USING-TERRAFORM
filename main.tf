...
resource "aws_s3_bucket" "request_bucket" {
  bucket = "richmond-translate-requests"
  force_destroy = true
}
 
# Response bucket (output JSON)
resource "aws_s3_bucket" "response_bucket" {
  bucket = "richmond-translate-responses"
  force_destroy = true
}
...
resource "aws_iam_role" "lambda_role" {
  name = "richmond-translate-lambda-role"
...
resource "aws_iam_role_policy" "lambda_policy" {
  name = "richmond-translate-policy"
...
resource "aws_lambda_function" "translate_lambda" {
  function_name = "richmond-translate-lambda"
